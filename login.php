...
<title>Prijavljivanje korisnika | Besplatni mali oglasi</title>
...
<meta property="og:title" content="Prijavljivanje korisnika | Besplatni mali oglasi">
<meta property="og:site_name" content="Besplatni Oglasi">
<meta property="og:url" content="http://besplatni-oglasi.esy.es/prijavljivanje">
...
try	{
	...
	if(!$captcha){
		echo '<p>Please check the the captcha form.</p>';
...
if( password_verify ( $password , $retrived_password )){
	...
	$last_login = date("Y-m-d H:i:s");
		$sql = ' update users set last_login = :last_login where email = :email limit 1 ';
		$last_login_update = $db -> prepare ( $sql ) ;
		$last_login_update -> execute ( array ( 'last_login' => $last_login , 'email' => $email ));
		
		$_SESSION['logged']= 1 ;
		$_SESSION['username'] = $retrived_password_result['username'];
		$_SESSION['email'] = $retrived_password_result['email'];
		$_SESSION['user_id'] = $retrived_password_result['user_id'];
		$_SESSION['access_lvl'] = $retrived_password_result['access_lvl'];
...
else
{
	...
	echo   '<p>Pogrešno ste uneli email adresu ili šifru.</p>';
...
<form class="form-horizontal" role="form"  method="post" action="/prijavljivanje" >
                ...
                <div class="form-group">
                    <label for="hidden" class="col-sm-3 control-label"></label>
                    <div class="col-sm-9">
				<div class="g-recaptcha" data-sitekey="6LeFbQUTAAAAANM-b5ibXIeP8dgZq6UGfGLK7U0T"></div><br>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <div class="col-sm-9 col-sm-offset-3">
                        <button type="submit" name="submit" class="btn btn-primary btn-block">Prijavi se</button>
                    </div>
                </div>
            </form> <!-- /form -->
